  ...
  return (
    <div className="legal-page-theme" style={{ backgroundColor: 'var(--background)', color: 'var(--foreground)', minHeight: '100vh', display: 'flex', flexDirection: 'column', overflowX: 'hidden' }}>
      ...
      <main style={{ flex: 1, position: 'relative' }}>
        ...
        <div className="decorative-blur" style={{ top: '100px', right: '-100px', background: 'var(--foreground)' }}></div>
        ...
        <section className="legal-header" style={{ direction: isRTL ? 'rtl' : 'ltr' }}>
          ...
          >
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.6rem', backgroundColor: 'var(--muted)', padding: '0.6rem 1.25rem', borderRadius: '9999px', border: '1px solid var(--border)', flexDirection: isRTL ? 'row-reverse' : 'row' }}>
              <ShieldCheck size={18} />
              <span style={{ fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.15em', fontSize: '10px' }}>{t.privacy.badge}</span>
            </div>
            <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: '900', letterSpacing: '-0.04em' }}>{t.privacy.title}</h1>
            <p 
        ...
        <div className="manual-layout" style={{ direction: 'ltr' }}>
          <aside className="sidebar-nav" style={{ textAlign: isRTL ? 'right' : 'left' }}>
            <p style={{ fontSize: '11px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '1rem', color: 'var(--muted-foreground)', padding: isRTL ? '0 1.25rem 0 0' : '0 0 0 1.25rem' }}>{t.privacy.onThisPage}</p>
            {t.privacy.sections.map((s, idx) => (
