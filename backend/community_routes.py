...
async def delete_comment(
    ...
    """Delete own comment."""
    user_id = get_current_user_id(authorization)
    comment = db.query(PostComment).filter(PostComment.comment_id == comment_id, PostComment.user_id == user_id).first()
    if not comment:
    ...
    # Update cached comments_count on Post
    post = db.query(Post).filter(Post.post_id == post_id).first()
    if post:
        post.comments_count = db.query(func.count(PostComment.comment_id)).filter(PostComment.post_id == post_id).scalar()
        db.commit()
